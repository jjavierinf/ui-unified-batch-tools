...
class StarRocksSparkConnector:

    """
    A class for starrocks_connecting to and interacting with StarRocks using PySpark.
    ...
    def write_to_starrocks(self,starrocks_conn, df: DataFrame, database: str, table: str) -> None:
        """
        ...
        options = {
            "starrocks.fe.http.url": f"{starrocks_conn['host']}:8030",
            "starrocks.fe.jdbc.url": f"jdbc:mysql://{starrocks_conn['host']}:9030/{database}",
            "starrocks.user": starrocks_conn['user'],
            "starrocks.password": starrocks_conn['password'],
    ...
    def read_from_starrocks(self, database: str, table: str) -> DataFrame:
        ...
        options = {
            ...
            "spark.starrocks.read.fe.urls.http": self.starrocks_connection["url"],
            "spark.starrocks.read.fe.urls.jdbc": f'jdbc:mysql://{self.starrocks_connection["url"]}/{database}',
            "spark.starrocks.read.database": database,
            "spark.starrocks.read.table": table,
            "spark.starrocks.read.username": self.starrocks_connection["username"],
            "spark.starrocks.read.password": self.starrocks_connection["password"],
        }
